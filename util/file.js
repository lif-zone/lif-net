...
E.fread_line_cb_e = (fd, cb, opt)=>{
  opt = {encoding: 'utf8', buf_size: E.read_buf_size, ...opt};
  cb = cb||(()=>false);
...
function fix_data(data){ return typeof data=='number' ? ''+data : data; }
...
E.rm_rf_e = dir=>fs.rmSync(dir, {recursive: true, force: true});
...
E.readdir_r_e = (dir, opt)=>{
  ...
  return E.find_e(dir, {strip: new RegExp('^'+dir+'/'), ...opt});
};
...
E.normalize = p=>E.win2unix(path.normalize(p));
