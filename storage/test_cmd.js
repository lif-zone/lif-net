...
function state_split_var(v, def){
  let o = parse_var(v), {type, seq, cfid, index, id, key} = o;
  if (o.def)
    set_def('left', o.ctx);
...
const index_find = (scroll, filter)=>etask(function*index_find(){
  ...
  for (let i=0; i<filter.length; i++){
    let t = parse_exp_arg(filter[i]), id, key, min, max, dir, count, bseq;
    let cfid, name;
    ...
    for (let curr=t.r; curr = parse_get_next(curr);){
      ...
      case 'min': min = +tt.r; break;
      case 'count': count = +tt.r; break;
      case 'bseq': bseq = tt.r; break;
      case 'cfid': cfid = +tt.r; break;
      case 'name': name = tt.r; break;
      case 'dir': dir = tt.r; break;
      default: assert.fail('invalid index_find '+tt.l+' for '+filter[i]);
      }
    }
    if (bseq)
      cfid = cfid||0;
    let found = yield scroll.index_find(key, {id, cfid, bseq, min, max, dir,
      count, name});
    if (!found)
      continue;
    ret = ret||{};
